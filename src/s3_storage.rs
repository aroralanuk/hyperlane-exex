...
impl S3Storage {
    pub async fn write_to_bucket(&self, key: &str, body: &str) -> Result<()> {
        ...
        let put_request = client.put_object().bucket(self.bucket.clone()).key(key).body(ByteStream::from(body.as_bytes().to_vec())).content_type("application/json");
    ...
    async fn get_authenticated_client(&self) -> Result<&Client> {
        let key_id = env::var(ENV_CRED_KEY_ID)
            .context("Missing AWS_ACCESS_KEY_ID")?;
        let key_secret = env::var(ENV_CRED_KEY_SECRET)
            .context("Missing AWS_SECRET_ACCESS_KEY")?;
